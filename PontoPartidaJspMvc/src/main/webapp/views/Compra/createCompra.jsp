...
<body>
				...
				<div class="collapse navbar-collapse" id="navbar-items">
					<ul class="navbar-nav me-auto mb-2 mb-lg-0" id="navbar-li">
						...
						<li class="nav-item"><a class="nav-link active"
							aria-current="page" href="createCompra.jsp">Compra</a></li>
					</ul>
				</div>
			</div>
		</nav>
	...
	<main class="container" id="slider">
		...
		<section class="container py-3 bg-secondary-color my-5">
			<!-- QUAL O DESTINO????????????????????????????????????????????????????????????????????????????????????????????????????????????????????? -->
			<form method="get" action="../../createPessoa">

				<h4 class="text-center">Compra</h4>

				<div class="form-group mb-3">
					<label for="inlineFormCustomSelectPref">Pacote</label> <select
						class="custom-select my-1 mr-sm-2" id="ItensCompra"
						name="ItensCompra">
						<option selected="">Selecione...</option>
						<option value="1">Flamengo X Botafogo - Campeonato
							Carioca</option>
						<option value="2">Gremio X Palmeiras - Brasileirão</option>
					</select>
				</div>

				<div class="form-group mb-3">
					<label for="quantidadePacotes" class="form-label">
						Quantidade de Pacotes </label> <input type="number" id="quantidadePacotes"
						name="quantidadePacotes" class="form-control"
						value="<%=request.getAttribute("quantidadePacotes")%>" />
				</div>

				<div class="form-group mb-3">
					<label class="form-label">Valor Total da Compra: R$900,00</label> <input
						type="hidden" id="valorCompra" name="valorCompra"
						class="form-control" value="valorCompra" />
				</div>

				<div class="form-group mb-3">
					<label class="form-label">Cliente: Fulano de Tal</label> <input
						type="hidden" id="pessoa" name="pessoa" class="form-control"
						value="pessoa" />
				</div>

				<button type="submit" class="btn btn-primary">Confirmar Compra</button>
				<a href="createCompra.jsp" class="btn btn-danger"
