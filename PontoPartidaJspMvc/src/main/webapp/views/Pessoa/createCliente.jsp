...
<!-- QUAL O DESTINO????????????????????????????????????????????????????????????????????????????????????????????????????????????????????? -->	
			<form method="get" action="../../createPessoa">
				...
				<div class="form-group mb-3">
					<label for="nomePessoa" class="form-label"> Nome </label> <input
						type="text" id="nomePessoa" name="nomePessoa" class="form-control" />
				</div>
				...
				<div class="form-group mb-3">
					<label for="inlineFormCustomSelectPref">Sexo</label> <select
				...
				<input type="hidden" id="permissao" name="permissao"
					class="form-control" value="cliente" />
				...
				<a href="createPessoa.jsp" class="btn btn-danger" style="margin-left: 10px">
					Cancelar </a>
			</form>
		</section>
