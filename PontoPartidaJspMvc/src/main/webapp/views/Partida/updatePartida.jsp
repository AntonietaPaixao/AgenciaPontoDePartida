...
<body>
	...
	<main class="container" id="slider">
		...
		<section class="container py-3 bg-secondary-color my-5">
			<!-- QUAL O DESTINO????????????????????????????????????????????????????????????????????????????????????????????????????????????????????? -->
			<form action="./edit-Partida">
				<h2 class="text-center">Atualizar Partida</h2>

				<div class="form-group mb-3">
					<label for="idPartida" class="form-label"> ID Partida </label> <input
						type="number" id="idPartida" name="idPartida" class="form-control"
						value="<%=request.getAttribute("idPartida")%>" />
				</div>

				<div class="form-group mb-3">
					<label for="dataPartida" class="form-label"> Data da
						Partida </label> <input type="text" id="dataPartida" name="dataPartida"
						class="form-control" value="<%=request.getAttribute("dataPartida")%>"/>
				</div>

				<div class="form-group mb-3">
					<label for="nomeCampeonato" class="form-label">Campeonato </label>
					<input type="text" id="nomeCampeonato" name="nomeCampeonato"
						class="form-control" value="<%=request.getAttribute("nomeCampeonato")%>"/>
				</div>

				<div class="form-group mb-3">
					<label for="timeCasa" class="form-label">Time da Casa </label> <input
						type="text" id="timeCasa" name="timeCasa" class="form-control"
						value="<%=request.getAttribute("timeCasa")%>" />
				</div>

				<div class="form-group mb-3">
					<label for="timeVisitante" class="form-label"> Time
						Visitante </label> <input type="text" id="timeVisitante"
						name="timeVisitante" class="form-control" 
						value="<%=request.getAttribute("timeVisitante")%>"/>
				</div>
	
				<button type="submit" class="btn btn-primary">Enviar</button>
				<a href="updatePartida.jsp" class="btn btn-danger"
				...
				<div class="col-12" id="footer-details">
					<div class="row">
						<div class="col-12 col-md-4" id="news-container">
							<h4>Saiba o que estão rolando em campo</h4>
